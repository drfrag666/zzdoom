...
namespace swrenderer
{
	...
	void RenderWallPart::Render(const sector_t* lightsector, seg_t* curline, const FWallCoords& WallC, FTexture* pic, int x1, int x2, const short* walltop, const short* wallbottom, const ProjectedWallTexcoords& texcoords, bool mask, bool additive, fixed_t alpha)
	{
		if (pic == nullptr)
			return;
	...
	void RenderWallPart::ProcessNormalWall(const short* uwal, const short* dwal, const ProjectedWallTexcoords& texcoords)
	{
		...
		if (!pic->bMasked && mask && alpha >= OPAQUE && !additive)
		{
		...
		drawerargs.texwidth = pic->GetWidth();
		drawerargs.texheight = pic->GetHeight();
		...
		else
		{
			...
			int fracbits = 32 - pic->HeightBits;
