...
namespace swrenderer
{
	...
	void SWRenderLine::SetTopTexture()
	{
		...
		mTopTexture = TexMan(sidedef->GetTexture(side_t::top), true);
	}
	...
	void SWRenderLine::SetMiddleTexture()
	{
		...
		mMiddleTexture = TexMan(sidedef->GetTexture(side_t::mid), true);
	}
	...
	void SWRenderLine::SetBottomTexture()
	{
		...
		mBottomTexture = TexMan(sidedef->GetTexture(side_t::bottom), true);
	}
	...
	void SWRenderLine::MarkOpaquePassClip(int x1, int x2)
	{
		...
		{ // two sided line
			if (mTopTexture != nullptr && mTopTexture->UseType != ETextureType::Null)
			{ // top wall
				for (int x = x1; x < x2; ++x)
				{
					wallupper.ScreenY[x] = MAX(MIN(wallupper.ScreenY[x], floorclip[x]), walltop.ScreenY[x]);
			...
			if (mBottomTexture != nullptr && mBottomTexture->UseType != ETextureType::Null)
			{ // bottom wall
				for (int x = x1; x < x2; ++x)
				{
					walllower.ScreenY[x] = MIN(MAX(walllower.ScreenY[x], ceilingclip[x]), wallbottom.ScreenY[x]);
	...
	void SWRenderLine::RenderTopTexture(int x1, int x2)
	{
		...
		if (!mTopTexture || mTopTexture->UseType == ETextureType::Null) return;
	...
	{
		if (!mMiddleTexture || mMiddleTexture->UseType == ETextureType::Null) return;
		if (!viewactive) return;
	...
	{
		if (mMiddleTexture) return;
		if (!mBottomTexture || mBottomTexture->UseType == ETextureType::Null) return;
		if (!viewactive) return;
