...
inline bool P_IsBlockedByLine(AActor* actor, line_t* line)
{
		...
		|| ((i_compatflags & COMPATF_NOBLOCKFRIENDS) && (actor->flags & MF_FRIENDLY))))
	{
		...
		if ((line->flags2 & ML2_BLOCKLANDMONSTERS) && level.MBF21Enabled() && !(actor->flags & MF_FLOAT)) return true;
	}

	// Blocking players
	if ((actor->player != nullptr) && (line->flags & ML_BLOCK_PLAYERS) && level.MBF21Enabled()) return true;
