...
namespace swrenderer
{
	void RenderWallPart::ProcessNormalWall(const short *uwal, const short *dwal, double texturemid, float *swal, fixed_t *lwal)
	{
		...
		if (!rw_pic->bMasked && mask && alpha >= OPAQUE && !additive)
		{
		...
		if (viewport->RenderTarget->IsBgra())
		{
			for (int x = x1; x < x2; x++, curlight += lightstep)
			{
				...
				fixed_t xxoffset = (lwal[x] + xoffset + FLOAT2FIXED(xmagnitude * 0.5));
				...
				int mipmap_offset = 0;
				int mip_width = rw_pic->GetWidth();
				int mip_height = rw_pic->GetHeight();
		...
		{
			uint32_t height = rw_pic->GetHeight();

			uint32_t uv_max;
			int uv_fracbits = 32 - rw_pic->HeightBits;
			if (uv_fracbits != 32)
			...
			for (int x = x1; x < x2; x++, curlight += lightstep)
			{
				...
				int width;
				if (col < 0 && (width = rw_pic->GetWidth()) != (1 << rw_pic->HeightBits))
				{
					col = width + (col % width);
				}
